...
int moveMaxToFront(ListNode **ptrHead)
{
  ...
  ListNode *beforeHighestNode = NULL;
  ListNode *highestNode = curNode;
  while(curNode != NULL){
    if(curNode->item > highestNode->item){
      highestNode = curNode;
      beforeHighestNode = preNode;
    }
    preNode = curNode;
    curNode = curNode->next;
  }

  if(beforeHighestNode != NULL){
    beforeHighestNode->next = highestNode->next;
    highestNode->next = *ptrHead;
    *ptrHead = highestNode;
  }
