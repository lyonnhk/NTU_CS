...
int removeNode(ListNode **ptrHead, int index){
    ...
    prevNode = malloc(sizeof(ListNode));
    int count = 0;

    if (*ptrHead == NULL || index < 0 || findNode(*ptrHead, index) == NULL){
        return -1;
    }

    if(index == 0){
        printf("INSIDE RETURN 0 number 2 \n");
        currNode = *ptrHead;
    ...
    if ((prevNode = findNode(*ptrHead, index-1)) != NULL){
        printf("INSIDE RETURN 0 \n");
